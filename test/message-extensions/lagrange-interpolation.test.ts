...
import { getUnivariateLDE } from "../../lib/sum-check/univariate-lagrange.ts";
import { getMultilinearLDE } from "../../lib/sum-check/multilinear-lagrange.ts";
...
describe("Lagrange Interpolation Implementations should return the corresponding character in a message given the index", () => {
  it(`Case 1`, () => {
    ...
    message.split("").forEach((char, index) => {
      ...
      const univariateLDE = getUnivariateLDE(
        message.split("").map((c) => Field(asciiToNumber(c))),
        field,
      );
      const multilinearLDE = getMultilinearLDE(
        message.split("").map((c) => Field(asciiToNumber(c))),
        binaryVector,
      );
